...
namespace PBL3
{
    public partial class inforMember : Form
    {
        ...
        {
            string query = "SELECT Ho_ten as [Họ và tên], Ngay_sinh as [Ngày tháng năm sinh], So_dien_thoai as [Số điện thoại], Email as [Email], Gioi_tinh as [Giới tính], Ma_goi_tap as [Mã gói tập], Ngay_tham_gia as [Ngày tham gia], Ma_HLV as [Mã HLV], Dia_chi as [Địa chỉ]FROM Khach WHERE ID_khach = @ID_khach";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dateTimePicker1.Value = Convert.ToDateTime(row["Ngày tháng năm sinh"]);
                ...
                txtHLV.Text = row["Mã HLV"].ToString();
